...
def spherical_jn(
    ...
    """
    x_is_zero = x == 0.0
    x_no_zero = torch.where(x_is_zero, torch.ones_like(x), x)

    if angular_l == 0:
        results = torch.sin(x_no_zero) / x_no_zero
        results = torch.where(x_is_zero, torch.ones_like(x), results)
    elif angular_l == 1:
    ...
    if angular_l > 0:
        results = torch.where(x_is_zero, torch.zeros_like(x), results)

    return results


_PRECOMPUTED_SPHERICAL_BESSEL_ZEROS = torch.tensor(
    [
        # l = 0
        [
            3.14159265358979,
...
def spherical_bessel_zeros(
    ...
    return _PRECOMPUTED_SPHERICAL_BESSEL_ZEROS[angular_l, radial_n].item()
...
def real_spherical_harmonics_m0(
    ...
    if angular_l == 0:
        prefactor = 1.0 / math.sqrt(4.0 * math.pi)
        return cos_theta.new_full(cos_theta.shape, prefactor)
    if angular_l == 1:
        prefactor = math.sqrt(3.0 / (4.0 * math.pi))
        return cos_theta.new_full(cos_theta.shape, prefactor) * cos_theta
    if angular_l == 2:
        prefactor = math.sqrt(5.0 / (16.0 * math.pi))
        return prefactor * (3.0 * cos_theta.pow(2) - 1.0)
    if angular_l == 3:
        prefactor = math.sqrt(7.0 / (16.0 * math.pi))
        return prefactor * (5.0 * cos_theta.pow(3) - 3.0 * cos_theta)
    if angular_l == 4:
        prefactor = math.sqrt(9.0 / (256.0 * math.pi))
        return prefactor * (35.0 * cos_theta.pow(4) - 30.0 * cos_theta.pow(2) + 3.0)
...
class SphericalHarmonicAndRadialBasis(nn.Module):
    ...
    def forward(self, r: torch.Tensor, theta: torch.Tensor) -> torch.Tensor:
        ...
        normalization = math.sqrt(2.0 / self.cutoff**3)
        for angular_l in range(self.max_angular_l):
            angular_component = real_spherical_harmonics_m0(cos_theta, angular_l)
            for radial_n in range(self.max_radial_n):
                z_ln = spherical_bessel_zeros(angular_l, radial_n)
                z_ln_tensor = r.new_tensor(z_ln)
                prefactor = r.new_tensor(normalization) / spherical_jn(
                    angular_l + 1, z_ln_tensor
                )
                radial_component = spherical_jn(angular_l, z_ln_tensor * r_scaled)
                shrb[radial_n, angular_l, :] = (
                    prefactor * radial_component * angular_component
                )
